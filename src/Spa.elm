...
@docs static
@docs **Note:** Static pages don't store data, so Model is always an empty tuple: ()
...
@docs element
@docs **New to Cmd or Sub?** I recommend checking out Elm's official guide, that's a great place to wrap your head around these two concepts.
...
@docs component
@docs **Cool trick:** Don't need access to Global.Model in all of your functions? Use always to skip the first argument.
@docs In the following example, only the view function receives Global.Model:
@docs import Global
@docs import Spa exposing (Page)
@docs page : Page Flags Model Msg Global.Model Global.Msg
@docs page =
@docs Spa.component
@docs { init = always init
@docs update = always update
@docs view = view -- not using "always"!
@docs subscriptions = always subscriptions
@docs }
...
3.  `/faqs` - frequently asked questions
    ...
    type Model
    = Home\_Model Pages.Home.Model
    | Menu\_Model Pages.Menu.Model
    | Faqs\_Model Pages.Faqs.Model
    type Msg
    = Home\_Msg Pages.Home.Msg
    | Menu\_Msg Pages.Menu.Msg
    | Faqs\_Msg Pages.Faqs.Msg
...
2.  `Home_Msg` can upgrade a `Pages.Home.Msg` to a `Msg`
    ...
    @docs upgrade
    @docs import Pages.Faqs
    @docs import Pages.Home
    @docs import Pages.Menu
    @docs import Spa
    @docs type alias UpgradedPage pageFlags pageModel pageMsg =
    @docs { init : pageFlags -> Global.Model -> ( Model, Cmd Msg, Cmd Global.Msg )
    @docs update : pageMsg -> pageModel -> Global.Model -> ( Model, Cmd Msg, Cmd Global.Msg )
    @docs bundle : pageModel -> Global.Model -> Spa.Bundle Msg
    @docs }
    @docs type alias UpgradedPages =
    @docs { home : UpgradedPage Pages.Home.Flags Pages.Home.Model Pages.Home.Msg
    @docs menu : UpgradedPage Pages.Menu.Flags Pages.Menu.Model Pages.Menu.Msg
    @docs faqs : UpgradedPage Pages.Faqs.Flags Pages.Faqs.Model Pages.Faqs.Msg
    @docs }
    @docs pages : UpgradedPages
    @docs pages =
    @docs { home = Pages.Home.page |> Spa.upgrade Home_Model Home_Msg
    @docs menu = Pages.Menu.page |> Spa.upgrade Menu_Model Menu_Msg
    @docs faqs = Pages.Faqs.page |> Spa.upgrade Faqs_Model Faqs_Msg
    @docs }
    @docs Now when we write `init`, we can use the upgraded `pages` variable to keep things easy to read.
...
pages : UpgradedPages
pages =
{ home = Pages.Home.page |> Spa.upgrade Home\_Model Home\_Msg
, menu = Pages.Menu.page |> Spa.upgrade Menu\_Model Menu\_Msg
, faqs = Pages.Faqs.page |> Spa.upgrade Faqs\_Model Faqs\_Msg
}
-}
upgrade :
    (pageModel -> model)
